...
import {
  ArrowLeft, Plus, Trash2, GripVertical, AlertTriangle,
  Link as LinkIcon, StickyNote, DollarSign, Calendar, Edit2,
  ChevronDown, ChevronUp, CircleDot, CheckCircle2,
} from "lucide-react";
import {
...
export default function ProjectDetail({ project, onBack, onUpdate }: Props) {
  ...
  const saveTask = async () => {
    ...
    if (editingTask) {
      tasks = project.tasks.map(t => t.id === editingTask.id ? {
        ...
        column: taskForm.column, priority: taskForm.priority, dueDate: taskForm.dueDate,
        completed: taskForm.column === 'done',
      } : t);
    } else {
      tasks = [...project.tasks, {
        id: newId(), title: taskForm.title, description: taskForm.description,
        column: taskForm.column, priority: taskForm.priority, dueDate: taskForm.dueDate,
        completed: taskForm.column === 'done', createdAt: new Date().toISOString(),
      }];
    }
  ...
  const moveTask = async (taskId: string, newColumn: TaskColumn) => {
    const tasks = project.tasks.map(t => t.id === taskId ? { ...t, column: newColumn, completed: newColumn === 'done' } : t);
    await onUpdate({ ...project, tasks });
  };
  ...
  const saveLink = async () => {
    ...
    const links = [...project.links, { id: newId(), title: linkForm.title || linkForm.url, url: linkForm.url }];
  ...
  const savePayment = async () => {
    ...
    const payments = [...(project.payments || []), {
      id: newId(), description: paymentForm.description, value: val, date: paymentForm.date,
    }];
    await onUpdate({ ...project, payments, paidAmount: totalPaid + val });
    toast({ title: "Pagamento registrado!" });
    setPaymentForm({ description: '', value: '', date: new Date().toISOString().split('T')[0] });
    setPaymentModal(false);
  };

  const deletePayment = async (id: string) => {
    const payments = (project.payments || []).filter(p => p.id !== id);
    const newTotal = payments.reduce((sum, p) => sum + p.value, 0);
    await onUpdate({ ...project, payments, paidAmount: newTotal });
  };
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-card border border-border rounded-xl p-3 text-center">
          ...
          <p className="text-foreground font-bold text-sm">{formatCurrency(project.value)}</p>
          <p className="text-muted-foreground text-[10px]">Valor Total</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-3 text-center">
          <DollarSign className="w-4 h-4 text-emerald-400 mx-auto mb-1" />
          <p className="text-emerald-400 font-bold text-sm">{formatCurrency(totalPaid)}</p>
          <p className="text-muted-foreground text-[10px]">Pago</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-3 text-center">
          <DollarSign className="w-4 h-4 text-orange-400 mx-auto mb-1" />
          <p className={cn("font-bold text-sm", remaining > 0 ? "text-orange-400" : "text-emerald-400")}>{formatCurrency(Math.max(0, remaining))}</p>
          <p className="text-muted-foreground text-[10px]">Falta Pagar</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-3 text-center">
          <CheckCircle2 className="w-4 h-4 text-violet-400 mx-auto mb-1" />
          <p className="text-foreground font-bold text-sm">{completedTasks}/{project.tasks.length}</p>
          <p className="text-muted-foreground text-[10px]">Tarefas</p>
        </div>
      </div>

      {/* Progress bar */}
      {project.tasks.length > 0 && <Progress value={progress} className="h-2" />}
      ...
      {urgentTasks.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-red-400 text-xs font-bold">{urgentTasks.length} tarefa(s) urgente(s)</span>
          </div>
      ...
      <Tabs defaultValue="kanban" className="w-full">
        <TabsList className="w-full justify-start overflow-x-auto">
          ...
          <TabsTrigger value="notas">Notas ({project.notes.length})</TabsTrigger>
          <TabsTrigger value="links">Links ({project.links.length})</TabsTrigger>
        ...
        <TabsContent value="kanban" className="mt-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {COLUMNS.map(col => {
              ...
              return (
                <div key={col} className={cn("bg-secondary/30 rounded-xl border-t-2 p-2 min-h-[120px]", TASK_COLUMN_COLORS[col])}>
                  ...
                  <div className="space-y-2">
                    {colTasks.map(task => (
                      <motion.div
                        key={task.id}
                        layout
                        className="bg-card border border-border rounded-lg p-2.5 cursor-pointer hover:border-primary/40 transition-colors group"
                        onClick={() => openEditTask(task)}
                      >
                        <div className="flex items-start justify-between gap-1">
                          <p className="text-foreground text-xs font-medium leading-tight flex-1">{task.title}</p>
                          <button
                        ...
                        <div className="flex gap-1 mt-2 flex-wrap">
                          {COLUMNS.filter(c => c !== col).map(c => (
                            <button
                              key={c}
                              onClick={(e) => { e.stopPropagation(); moveTask(task.id, c); }}
                              className="text-[9px] px-1.5 py-0.5 rounded bg-secondary text-muted-foreground hover:text-foreground hover:bg-secondary/80 transition-colors"
                            >
        ...
        <TabsContent value="notas" className="mt-3 space-y-3">
          <div className="flex justify-end">
            <Button size="sm" onClick={openNewNote} className="gap-1"><Plus className="w-4 h-4" /> Nova Nota</Button>
          </div>
        ...
        <TabsContent value="links" className="mt-3 space-y-3">
          <div className="flex justify-end">
            <Button size="sm" onClick={() => setLinkModal(true)} className="gap-1"><Plus className="w-4 h-4" /> Novo Link</Button>
          </div>
            ...
            <div className="space-y-2">
              {project.links.map(link => (
                <div key={link.id} className="flex items-center gap-3 bg-secondary/50 rounded-lg p-3 group">
                  ...
                  <button onClick={() => deleteLink(link.id)} className="opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        ...
        <TabsContent value="financeiro" className="mt-3 space-y-4">
          <div className="flex justify-end">
            <Button size="sm" onClick={() => setPaymentModal(true)} className="gap-1"><Plus className="w-4 h-4" /> Novo Pagamento</Button>
          </div>
          ...
          {(project.payments || []).length === 0 ? (
            ...
            <div className="space-y-2">
              {[...(project.payments || [])].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(payment => (
                <div key={payment.id} className="flex items-center gap-3 bg-secondary/50 rounded-lg p-3 group">
                  <DollarSign className="w-4 h-4 text-emerald-400 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-foreground text-sm font-medium">{payment.description}</p>
                    <p className="text-muted-foreground text-[10px]">
                      {new Date(payment.date).toLocaleDateString('pt-BR')}
                    </p>
                  </div>
                  <span className="text-emerald-400 font-bold text-sm shrink-0">{formatCurrency(payment.value)}</span>
                  <button onClick={() => deletePayment(payment.id)} className="opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <Trash2 className="w-4 h-4 text-muted-foreground hover:text-destructive" />
                  </button>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={taskForm.column} onValueChange={v => setTaskForm({ ...taskForm, column: v as TaskColumn })}>
                  ...
                  <SelectContent>
                    {COLUMNS.map(c => <SelectItem key={c} value={c}>{TASK_COLUMN_LABELS[c]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
